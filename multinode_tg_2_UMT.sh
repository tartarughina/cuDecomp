...
size=256
# Log dir path
log_path="/home/tartarughina/cuDecomp/tg_log"
...
# Execute 5 times
for i in {1..5}; do
    mpiexec --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./tg -n "${size}" -u -t > "${log_path}/UMT_gpus_${NTOTRANKS}_size_${size}_iter_${i}.txt"
done
