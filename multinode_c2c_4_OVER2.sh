...
# Execute 5 times
for i in {1..5}; do
    mpiexec --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./benchmark_c2c -b 4 -x ${size} -y ${size} -z ${size} -m --oversub 2 > "${log_path}/OVER2_gpus_${NTOTRANKS}_size_${size}_iter_${i}.txt"
done
