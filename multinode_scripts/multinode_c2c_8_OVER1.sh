...
# Execute 5 times
for i in {1..5}; do
    mpirun --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./benchmark_c2c -b 4 -x ${size} -y ${size} -z ${size} -m --oversub 1 > "${log_path}/OVER1_gpus_${NTOTRANKS}_size_${size}_iter_${i}.txt"
done
