...
size=256
# Log dir path
log_path="/home/tartarughina/cuDecomp/tg_over_log"
...
cd /home/tartarughina/cuDecomp/bin/examples/cc/taylor_green

# Execute 5 times
for i in {1..5}; do
    mpiexec --envall --np "${NTOTRANKS}" --ppn "${nranks}" --hostfile "$PBS_NODEFILE" --cpu-bind list:0,8,16,24 ./tg -n "${size}" -u --oversub 1 > "${log_path}/OVER1_gpus_${NTOTRANKS}_size_${size}_iter_${i}.txt"
done
